...
var Tpl = {//{{{
    item: Util.tpl(Path.resolve(__dirname, "helpItem.jade")),
    index: Util.tpl(Path.resolve(__dirname, "helpIndex.jade")),
};//}}}
...
function itemModelParser(prefs, spc, srvName, fName, fltIndex) {//{{{
    ...
    model.title = spc.path+" ("+Util.propGet(spc, "meta.title")+")";
    model.srvName = spc.srvName;
    model.meta = spc.meta;
    model.path = spc.path;
    model.name = fName;
